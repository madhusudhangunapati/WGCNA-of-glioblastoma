...
#the gene expression data has been loaded.

#in the data the columns where the patients ids and the rows where the genes which are expressed .
...
#before normalization we plot a heatmap of unnormalised data 
...
#next we calculate the adjacency matrix and topologicalsimilarity matrix and dissTOM
softPower<-9
...
#calculation of dissTOM
dissTOM=1-TOM



#next we cluster the data using hclust function
geneTree <- hclust(dist(quant_normgene.exp), method = "average");
plot(geneTree, xlab = "" , sub = "" , main = "hierarchial clustering of gene expression data" ,hang = 0.2);
...
#geneenrichment analaysis should be done 
library("org.Hs.eg.db")

GOenr = GOenrichmentAnalysis("moduleColors" , organism = "human", nBestP = 10);
...
#trying to heatmap the quantile normalised data in a different way more colourful 
heatmap(quant_normgene.exp , scale="none" , col=cm.colors(256))

#trying to heatmap the quantile normalised data using RcolorBrewer palette (using different colors)
coul<-colorRampPalette(brewer.pal(8, "PiYG"))(25)
heatmap(quant_normgene.exp , scale = "none", col=coul)

#trying to heatmap the standard normalise data in colorful way using cm colors 
heatmap(Ngene.exp ,scale="none" , col=cm.colors(256))

#trying to heatmap the unnormalised data with cm colors 
heatmap(df2 , scale ="row" , col=cm.colors(256))

#trying to heat map the standard normilised data using RcolorBrewer palette 
coul<-colorRampPalette(brewer.pal(8, "PiYG"))(25)
heatmap(Ngene.exp , scale = "none", col=coul)

#trying to heatmap the unnormalised gene expression data using Rcolorbrewer palette 
coul<-colorRampPalette(brewer.pal(8, "PiYG"))(25)
heatmap(df2, scale = "row", col=coul)





#trying to plot the TOM plot 
...
geneexp.cor<-cor(datExpr, method = c("pearson"))

#corrplot 
library(corrplot)
corrplot(geneexp.cor, order = 'hclust', 
         addrect = 10, addCoef.col = 'white', 
         number.cex = 0.7) 

geneexp.cor<-cor(quant_normgenet.exp , method = c("pearson"))
...
# Calculate topological overlap anew: this could be done more efficiently by saving the TOM
# calculated during module detection, but let us do it again here.
dissTOM = 1-TOMsimilarityFromExpr(quant_normgenet.exp, power = 7);
# Transform dissTOM with a power to make moderately strong connections more visible in the heatmap
plotTOM = dissTOM^7;
# Set diagonal to NA for a nicer plot
diag(plotTOM) = NA;
...
#ConnectivityCut = 12040# number of most connected genes that will be considered 
...
plotMat(t(scale(datExpr[,colorh1==which.module ]) ),nrgcols=30,rlabels=T,
        ...
        title=which.module )
####################################
#requires much time around 30 min 
...
######choosing top hub gene in each module ############
...
######choosing one  hub gene in each module ############
